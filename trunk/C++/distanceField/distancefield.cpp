...
QImage DistanceField::toImageRGBTest( int color )
{
    real r = (real)(this->_radius) + 2.0 ;
    uint32 r2 = 2*this->_radius;
    QImage image( this->_w + r2 , this->_h + r2 , QImage::Format_ARGB32 );
    ...
    for ( int i = 0 ; i < (int)this->_w ; ++i )
    {
        for ( int j = 0 ; j < (int)this->_h ; ++j )
        {
            if( this->dx( i , j ) < INF_LOCAL )
            {
                ...
                {
                case 0:
                    red = (int)( sqrt( ( this->dx( i + u , j + v) * this->dx( i + u , j + v) + this->dy( i+ u , j+ v) * this->dy( i+ u , j+ v) ) *invMaxS) * 255.0f);
                    break;
                case 1:
                    green = (int)( ( ( this->dy((real)i + u , (real)j + v) + r ) *invMax)*255.0f);
                    break;
                case 2:
                    blue = (int)( ( ( this->dx( (real)i + u , (real)j + v) + r ) *invMax)*255.0f);
                    break;
                default:
                    ;
                }
